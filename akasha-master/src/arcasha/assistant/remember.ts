 ...
 * 注: 「と」は名前の一部の可能性（まこと 等）があるため除去しない。
 * 「〜と呼んで」形式は呼んでパターンが先に捕捉するので、ここに「と」が残ることはない。
 */
export function cleanName(raw: string): string {
  return raw
    .replace(/(?:です|だ|でした|ます|さん|ちゃん|くん|君)+[。、!！?？\s]*$/g, '')
    .trim();
}
...
export function extractRememberAll(query: string, isDuplicate: DupChecker): RememberedItem[] {
  ...
  // 「〜と呼んで」形式を先に評価し、「私の名前は太郎と呼んでください」で
  // 「太郎」だけを捕捉する（「私の名前は…と呼んで」専用パターンを挟む）。
  const namePatterns = [
    /私は\s*([ぁ-んァ-ヶー一-龠々a-zA-Z0-9・]+?)\s*と\s*(?:呼んで|言って|お呼び)/,
    /私(?:の)?名前(?:は|が)\s*([ぁ-んァ-ヶー一-龠々a-zA-Z0-9・]+?)\s*と\s*(?:呼んで|言って|お呼び)/,
  ...
  // 文の区切り（。！？や改行・カンマ）の直後から始めるようにして前文の末尾を取り込まない。
  // 「私は猫が好き」のような一人称接頭（私(は|も)）も許容して「猫」を捕捉する。
  // 「猫が好き、犬が苦手」のようにカンマ区切りで並ぶ複数の好みも捕捉する。
