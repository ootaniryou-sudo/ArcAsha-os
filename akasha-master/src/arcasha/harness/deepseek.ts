...
export interface DshAdapterOptions {
   ...
   * 既定: ローカル lockfile 解決の ACP サーバー bin（dsh-acp-demo）の存在判定。
   */
  probe?: (signal?: AbortSignal) => Promise<boolean>;
  /**
   * ACP サーバーの起動コマンド。既定: ローカル node_modules/.bin/dsh-acp-demo。
   * 未インストールなら null を返し、実行は infrastructure failure になる（Native フォールバック）。
   */
...
/** 既定プローブ: lockfile 解決済みの ACP サーバー bin の存在判定（軽量・AbortSignal 対応）。 */
async function defaultProbe(signal?: AbortSignal): Promise<boolean> {
  if (signal?.aborted) return false;
  return localAcpServerBin() !== null;
}
...
export class DeepSeekHarnessAdapter implements Harness {
  ...
  async *execute(task: HarnessTask, options?: HarnessExecuteOptions): AsyncIterable<HarnessEvent> {
    ...
    try {
      ...
      const promptP = client.prompt(task.text).then(
        (response) => ({ ok: true as const, response }),
        (error) => ({ ok: false as const, error }),
      );

      const chunks: string[] = [];
      // ACP 型を漏らさないため、stopReason のみの構造的部分型で扱う
      type PromptOutcome =
        | { kind: 'prompt'; ok: true; response: { stopReason: string } }
        | { kind: 'prompt'; ok: false; error: unknown };
      type RaceResult =
        | { kind: 'chunk'; text: string }
        | { kind: 'closed' }
        | PromptOutcome;
      for (;;) {
        const settled: RaceResult = await Promise.race([
          client.nextMessage().then((v): { kind: 'chunk'; text: string } | { kind: 'closed' } =>
            v.done ? { kind: 'closed' } : { kind: 'chunk', text: v.value },
          ),
          promptP.then((r): PromptOutcome =>
            r.ok
              ? { kind: 'prompt', ok: true, response: r.response }
              : { kind: 'prompt', ok: false, error: r.error },
          ),
        ]);
        if (settled.kind === 'closed') continue;
        if (settled.kind === 'chunk') {
          chunks.push(settled.text);
          yield {
            type: 'message',
            taskId: task.taskId,
            executionId,
        ...
        // prompt resolve 前に届いた残メッセージを掃き出す（順序保証の保険）
        for (;;) {
          const tail = client.tryNextMessage();
          if (tail === null) break;
          chunks.push(tail);
        }
        const stop = settled.response.stopReason;
        if (stop === 'cancelled') {
