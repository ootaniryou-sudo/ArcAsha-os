...
- **プロジェクトA「Akasha-Link」（仮称）** — エッジデバイス（WebGPU）での超低遅延・分散推論 / テンソル伝送エンジン
  - 切り出した（2026-08-27）: `akasha-link/` に集約（`client-web` / `kernel-native` / `PROTOCOL.md` を `git mv` で移動、履歴保持）
...
| ビルド・CI | 分割前はルート package.json も `.github/workflows/ci.yml` も akasha-master のみ委譲 | A には CI が無かった → 本 PR で `akasha-link` job 新設済み（cargo check+test / client-web build） |
| 共有契約 | `PROTOCOL.md`（48B ヘッダ + f32[] ペイロードのバイナリワイヤ）が唯一の両者共有物 | 所有を決める必要がある |
| 重複 | `akasha-master/public/worker-inference.js` と `akasha-link/client-web/public/worker-inference.js` は**ほぼ同一**（ビルド成果物・先頭コメント 1 行のみ相違、機能は等価） | 最初に切るべき縫い目 |
...
- **Phase 1 — 境界の明確化（モノレポのまま・非破壊）** ✅（2026-08-27 実施）
  - ✅ `worker-inference.js` の重複確認（`akasha-master/public/` と `akasha-link/client-web/public/` はほぼ同一・先頭コメント 1 行のみ相違）※一本化は B 側の参照整理後に実施
...
- [x] **レビュー対応**（PR #25: CodeRabbit / Cubic の指摘を解決 — 本 PR で変更した docs / CI / package.json は修正済み。移動のみの既存コードへの指摘は別途追跡）
- [ ] **Phase 3（git 物理分割）** の実施可否（ユーザー確認待ち・破壊的操作のため必須）
- [ ] A の PR #25 を squash merge → main pull → 回帰確認
